...
function showCartOfUser() {
  ...
  for (let i = 0; i < cartOfUser.length; i++) {
    ...
    htmlResult =
      htmlResult +
      `<tr data-product_id="${productCart.id}">
...
function totalMoneyTmp(cartUpdateQuantity) {
  ...
  for (let i = 0; i < cartUpdateQuantity.length; i++) {
    totalMoney =
      totalMoney +
      parseInt(cartUpdateQuantity[i].quantity) * cartUpdateQuantity[i].price;
  }
    ...
    if (confirmDelete) {
      ...
      for (let i = 0; i < users.length; i++) {
       if (users[i].status == "active") {
         users[i].cart =  cartUpdateQuantity;
...
function uuid() {
  return Math.floor(Math.random() * 1233165446456);
}
...
function handleCheckout(event) {
  ...
  const orders = {
    ...
    status: "wait",
    nameOrder: userIsLogginning.name,
  };
